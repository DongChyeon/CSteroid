...
Java 메모리 모델(JMM)에서 volatile이 제공하는 핵심 보장은 가시성(visibility)과 실행 순서 보장(happens-before)이다.<br>
한 스레드가 volatile 변수에 값을 쓰면, 이후 다른 스레드가 해당 변수를 읽을 때 반드시 그 변경을 관측할 수 있음(가시성)이 보장된다. 이를 통해 한 스레드의 변경 사항이 다른 스레드에서 오래된 값으로 보이는 문제를 방지한다.<br>

이러한 보장을 위해 JVM은 volatile 변수의 읽기, 쓰기 전후에 메모리 배리어(memory barrier)를 삽입하여, 컴파일러나 CPU가 명령어를 재정렬(reordering)하지 못하도록 제한한다.
