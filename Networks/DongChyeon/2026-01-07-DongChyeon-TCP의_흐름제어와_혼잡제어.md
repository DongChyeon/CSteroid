...
무한정 2배씩 늘리다보면 네트워크 혼잡이 발생할 수 있는데 이를 제어하기 위해 임계치(Slow Start Threshold)라는 개념을사용한다:
...
하지만 타임아웃은 네트워크의 왕복 시간(RTT)보다 여유롭게 설저오디기 때문에, 패킷이 유실된 시점부터 재전송을 시작할 때까지 송신기는 아무것도 하지 않고 기다려야했다.<br>
