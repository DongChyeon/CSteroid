...
```kotlin
enum class LessonCardAlignment {
    CENTER,      // 중앙 정렬
    LEFT_40,     // 좌측 끝단으로부터 40dp 간격
    LEFT_16,     // 좌측 끝단으로부터 16dp 간격
    RIGHT_40,    // 우측 끝단으로부터 40dp 간격
    RIGHT_16     // 우측 끝단으로부터 16dp 간격
}
...
```kotlin
// 파일: LessonCardAlignment.kt

companion object {
    /**
     * 챕터 번호와 해당 챕터 내 인덱스로 정렬 타입 계산
     *
     * @param chapterNumber 챕터 번호 (1부터 시작)
     * @param indexInChapter 챕터 내 레슨 인덱스 (0부터 시작)
     * @return 해당 위치의 정렬 타입
     */
    fun fromChapterAndIndex(chapterNumber: Int, indexInChapter: Int): LessonCardAlignment {
        val isOddChapter = chapterNumber % 2 == 1
...
```kotlin
// QuizMapScreen.kt에서 실제 사용

lessons.forEachIndexed { index, lesson ->
    val alignment = LessonCardAlignment.fromChapterAndIndex(
        chapterNumber = currentChapter,
        indexInChapter = index
    )
    val (paddingStart, paddingEnd) = alignment.getPadding()

    Box(
        modifier = Modifier.fillMaxWidth(),
        contentAlignment = alignment.getAlignment()
    ) {
        KatsbyLessonCard(
            state = lesson.toCardState(),
            alignment = alignment,
...
- Introduction to Algorithms (CLRS) - Dynamic Programming
- Android Jetpack Compose Layout System
